...
while True:
    ...
    if time_now == "0001": # 设置要执行的时间
        ...
        for i in range(len(messages_json)):
            ...
            value = (now_time, messages_json[i].get('provinceName'), messages_json[i].get('provinceShortName'),
                     ...
                     messages_json[i].get('locationId'), messages_json[i].get('statisticsData'))
            valuesList.append(value)
            cityValue = messages_json[i].get('cities')
            # print(cityValue)
            for j in range(len(cityValue)):
                cityValueList = (now_time, cityValue[j].get('cityName'), cityValue[j].get('currentConfirmedCount'),
                                 cityValue[j].get('confirmedCount'), cityValue[j].get('suspectedCount'),
                                 cityValue[j].get('curedCount'), cityValue[j].get('deadCount'),
                                 cityValue[j].get('locationId'), messages_json[i].get('provinceShortName'))
                # print(cityValueList)
                cityList.append(cityValueList)
            # cityList.append(cityValue)
        # db = pymysql.connect("localhost", "root", "root", "yiqing", charset='utf8')
        db = pymysql.connect(host="localhost", user="root", password="root", database="yiqing", charset="utf8mb4")
        cursor = db.cursor()
